...
from cw_utils.config import ASSETS, DATA_DIR
from cw_utils.state_store import compute_accuracy, get_bankroll, load_predictions
...
apify_log_path = DATA_DIR / "apify_usage.jsonl"
apify_call_count = 0
...
if apify_log_path.exists():
    with open(apify_log_path) as f:
        apify_rows = [json.loads(line) for line in f if line.strip()]
    apify_call_count = sum(1 for row in apify_rows if row.get("source") == "apify" and row.get("success"))
    apify_fallback_count = sum(1 for row in apify_rows if row.get("source") == "binance_fallback")
...
with st.expander("📊 Apify Usage Details & Cost Breakdown", expanded=False):
    ...
    **How Apify is used in this project:**
    - **Agent 2** calls the `jan.sirucek/binance-scraper` Apify actor to fetch 1,000 × 5-min OHLCV bars per asset
...
if all_preds:
    ...
    if "was_correct" in df.columns:
        df["result"] = df.apply(
            lambda r: "✅" if r.get("was_correct") else ("❌" if r.get("resolved") else "⏳"), axis=1
        )
...
ohlcv_candidates = [
    DATA_DIR / f"{chart_asset}_ohlcv.csv",
    DATA_DIR / f"{chart_asset.lower()}_ohlcv.csv",
    DATA_DIR / f"ohlcv_{chart_asset.lower()}.csv",
]
...
if ohlcv_parquet.exists():
    try:
        ...
        pass
if ohlcv_df is None:
    for candidate in ohlcv_candidates:
        if candidate.exists():
            ohlcv_df = pd.read_csv(candidate)
            break
