...
from cw_tools.kalshi_tools import get_15min_markets
from cw_tools.polymarket_tools import get_crypto_5min_markets
from cw_tools.hermes_tools import hermes_available, synthesize_with_hermes
from cw_utils.config import ASSETS, DATA_DIR, RESOLUTION_MINUTES
from cw_utils.logger import get_agent_logger
from cw_utils.state_store import (
    compute_accuracy,
    get_bankroll,
    load_predictions,
    mark_resolved,
        ...
        for pred in unresolved:
            ...
            if pred_ts:
                try:
                    ...
                    age_minutes = (datetime.now(timezone.utc) - pred_dt).total_seconds() / 60.0
                    if age_minutes < RESOLUTION_MINUTES:
                        continue
                except Exception:
                    # If parsing fails, do not block resolution logic.
                    pass
            ...
            # Skip flat/noise moves so we do not overstate accuracy.
            if abs(float(current_price) - float(old_price)) < 1e-9:
                continue
...
def run_feedback_loop(assets: list[str] = None) -> dict:
    ...
    # 5. Hermes-based adaptive feedback suggestions
    result["adaptive_actions"] = _build_adaptive_actions(result, assets)

    _print_feedback_summary(result)
    return result


def _build_adaptive_actions(loop_result: dict, assets: list[str]) -> dict:
    """
    Create adaptive strategy actions from feedback stats.
    Prefers Hermes synthesis when available and falls back to deterministic rules.
    """
    accuracy_stats = loop_result.get("accuracy_stats", {})
    all_stats = accuracy_stats.get("ALL", {})
    total = int(all_stats.get("total", 0) or 0)
    accuracy = float(all_stats.get("accuracy", 0.0) or 0.0)
    arb_count = len(loop_result.get("arbitrage_opportunities", []))

    fallback_actions = []
    if total >= 10 and accuracy < 0.45:
        fallback_actions.append("Lower MAX_BET_PCT by 20% until rolling accuracy recovers")
        fallback_actions.append("Raise confidence threshold in Agent 4 to reduce weak trades")
    if total >= 10 and accuracy > 0.6:
        fallback_actions.append("Keep current risk settings and monitor for stability")
    if arb_count > 0:
        fallback_actions.append("Prioritize arbitrage cases with abs_gap >= 0.10 before directional bets")
    if not fallback_actions:
        fallback_actions.append("No parameter changes suggested; continue collecting outcomes")
    ...
    payload = {
        ...
        "bankroll": accuracy_stats.get("bankroll"),
    ...
    if hermes_available():
        try:
            prompt = (
                "You are a trading strategy feedback agent. "
                "Given the loop stats below, return JSON with keys: "
                "strategy_note (string), actions (array of 2-4 short actions), "
                "risk_mode (one of: defensive, neutral, aggressive).\n\n"
                f"Feedback data:\n{json.dumps(payload, indent=2)}"
            )
            text = synthesize_with_hermes(
                prompt=prompt,
                system_prompt=(
                    "You are Hermes Agent in a prediction-market feedback loop. "
                    "Be concise, actionable, and output valid JSON only."
                ),
            )
            ...
            return {
                ...
                "actions": parsed.get("actions", fallback_actions),
                "risk_mode": parsed.get("risk_mode", "neutral"),
                "generated_at": datetime.now(timezone.utc).isoformat(),
            }
        except Exception as e:
            logger.warning(f"Hermes adaptive feedback failed, using fallback rules: {e}")

    return {
        "source": "rules",
        "strategy_note": "Rule-based adaptive feedback from latest accuracy and arbitrage stats.",
        "actions": fallback_actions,
        "risk_mode": "defensive" if (total >= 10 and accuracy < 0.45) else "neutral",
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }
