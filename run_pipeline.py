...
Coordinates all 5 agents in sequence, uses Hermes Agent for synthesis when
available, and falls back to direct OpenRouter completion otherwise.
...
from cw_utils.config import ASSETS, DATA_DIR, LOOP_INTERVAL, OPENROUTER_API_KEY, OPENROUTER_MODEL
from cw_utils.logger import get_logger
from cw_utils.state_store import get_bankroll, update_prediction

logger = get_logger("pipeline")


# ── Hermes Agent / OpenRouter Synthesis ───────────────────────────

def synthesize_with_llm(pipeline_result: dict) -> str:
    """
    Prefer Hermes Agent as the orchestration/synthesis framework, then fall
    back to direct OpenRouter completion if Hermes is unavailable.
    """
    ...
    try:
        run_assets = pipeline_result.get("assets") or ASSETS
        assets_data = {}
        for asset in run_assets:
            ...
            assets_data[asset] = {
                ...
                "model_backend": pred.get("backend"),
...
Provide: a brief market commentary, key trading signals, and one risk note."""
        ...
        if hermes_available():
            synthesis = synthesize_with_hermes(
                prompt=prompt,
                system_prompt="You are Hermes Agent embedded inside a crypto trading research pipeline.",
            )
            logger.info("Hermes synthesis complete")
            return synthesis
...
def run_once(assets: list[str] = None, retrain: bool = False) -> dict:
    ...
    try:
        ...
        for asset in assets:
            prediction = predictions.get(asset, {})
            position = positions.get(asset, {})
            prediction_id = prediction.get("prediction_id")
            if prediction_id and position:
                update_prediction(
                    prediction_id,
                    {
                        "bet_usd": position.get("bet_usd", 0.0),
                        "bet_pct": position.get("bet_pct", 0.0),
                        "bet_on": position.get("bet_on"),
                        "action": position.get("action"),
                        "market_prob": position.get("market_prob"),
    ...
    try:
        ...
        adaptive = feedback.get("adaptive_actions", {})
        actions = adaptive.get("actions", [])
        if actions:
            print(f"  🧠 Adaptive mode: {adaptive.get('risk_mode', 'neutral')} ({adaptive.get('source', 'rules')})")
